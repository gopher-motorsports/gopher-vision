...
if not os.path.exists(math_channels_CSVFile_path):  # Check if the file exists
    # Create the file
    with open(
        math_channels_CSVFile_path, mode="w", newline="", encoding="utf-8"
    ) as csv_file:
...
def load_config(file=None):
    ...
    if file:
        ...
        path = filedialog.askopenfilename(  # Gives trace trap
            title="Open GopherCAN configuration", filetypes=[("YAML", "*.yaml")]
        )
        if not path:
            return
    ...
    with dpg.group(tag="parameter_table", parent="tab-gophercan"):
        dpg.add_input_text(
            hint="Name", callback=lambda _, val: dpg.set_value("ptable", val)
        )
    ...
    for preset in presets:
        dpg.add_selectable(
            parent="offline_presets_list_delete",
            label=preset,
            filter_key=preset,
            callback=delete_preset,
            user_data=preset,
        )

    # Read and add saved math channels from csv file
    with open(
        math_channels_CSVFile_path, mode="r", newline="", encoding="utf-8"
    ) as csv_file:
        ...
        for row in reader:
            ...
            math_channels_dict[row[0]]["placeholder_equation"] = ast.literal_eval(
                row[1]
            )
    ...
    for channel in math_channels_dict.keys():
        dpg.add_selectable(
            parent="math_channels_list",
            label=channel,
            filter_key=channel,
            callback=add_plot_math,
            user_data=channel,
        )
...
def convert():
    ...
    paths = filedialog.askopenfilename(
        title="Select Data", filetypes=[("GDAT", "*.gdat")], multiple=True
    )

    n = 0
    dpg.configure_item(
        "convert_loading", default_value=0.05, overlay=f"{n}/{len(paths)}"
    )
    for path in paths:
        path = Path(path)
        try:
...
def start_recording(sender, _):
    ...
    path = filedialog.asksaveasfilename(
        title="Record data", filetypes=[("GDAT", "*.gdat")], defaultextension="gdat"
    )
    root.destroy()
...
def stop_recording(sender, _):
    ...
    node.close_record()
    dpg.configure_item(
        "record_path", default_value="Not recording", color=COLORS["red"]
    )
...
def set_port_serial(sender, port_name):
    ...
    try:
        dpg.configure_item(
            "port_status",
            default_value=f"opening {port_name} ...",
            color=COLORS["gray"],
        )
        node.rx_port.open_serial(port_name)
        dpg.configure_item(
            "port_status", default_value=f"{port_name} open", color=COLORS["green"]
        )
    except Exception as err:
        print(err)
        dpg.configure_item(
            "port_status", default_value="No port open", color=COLORS["red"]
        )
...
def set_port_socket(sender, _, host=None, port=None):
    ...
    try:
        dpg.configure_item(
            "port_status",
            default_value=f"opening {host}:{port} ...",
            color=COLORS["gray"],
        )
        node.rx_port.bind_socket(host, port)
        dpg.configure_item(
            "port_status", default_value=f"{host}:{port} open", color=COLORS["green"]
        )
    except Exception as err:
        print(err)
        dpg.configure_item(
...
def trackside_connect(sender, _):
    ...
    try:
        ...
        dpg.configure_item(
            "port_status", default_value="Connection failed", color=COLORS["red"]
        )
        return
...
def toggle_mode(sender):
    ...
    if toggle == 0:
        light_theme = (
            create_theme_imgui_light()
        )  # Imports light mode from dearpygui_ext
...
def save_preset(sender):
    with dpg.window(
        label="Preset Name", tag="get_preset_name_window", width=300, height=200
    ):
...
def save_preset_to_csv(sender):
    ...
    math_channels = [
        alias[7:]
        for alias in dpg.get_aliases()
        if "p_plot_" in alias and alias[7:] in math_channels_dict
    ]
    ...
    with open(file_path, mode="w", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["id", "name", "y_min", "y_max"],
            extrasaction="ignore",
            lineterminator="\n",
        )
        writer.writeheader()
    ...
    dpg.add_selectable(
        parent="offline_presets_list",
        label=new_file_name,
        filter_key=new_file_name,
        callback=load_preset,
        user_data=new_file_name,
    )
    dpg.add_selectable(
        parent="offline_presets_list_delete",
        label=new_file_name,
...
def math_btn_callback(sender):
    ...
    if dpg.does_item_exist("math_channel_window"):
        dpg.delete_item("math_channel_window")
    if dpg.does_item_exist("math_channel_parameter_list"):
        dpg.delete_item("math_channel_parameter_list")
    if dpg.does_item_exist("math_channel_operators"):
        dpg.delete_item("math_channel_operators")

    with dpg.window(
        label="Create Custom Parameter",
        tag="math_channel_window",
        width=screen_width / 2,
        height=300,
    ):
        with dpg.group(horizontal=True, parent="math_channel_window"):
            dpg.add_button(
                tag="clear_custom_parameter_equation",
                label="Clear",
                callback=clear_custom_parameter_equation,
            )
        ...
        # list of parameters
        with dpg.popup(
            "choose_parameters_btn", no_move=True, mousebutton=dpg.mvMouseButton_Left
        ):
            dpg.add_input_text(
                hint="Name",
                callback=lambda _, val: dpg.set_value(
                    "math_channel_parameter_list", val
                ),
            )
        ...
        for parameter in parameters.values():
            dpg.add_selectable(
                parent="math_channel_parameter_list",
                label=parameter["name"],
                filter_key=parameter["name"],
                callback=save_parameter_name,
                user_data=parameter["id"],
            )
        ...
        # list of operators
        with dpg.popup(
            "choose_operator_btn", no_move=True, mousebutton=dpg.mvMouseButton_Left
        ):
            dpg.add_input_text(
                hint="Operator",
                callback=lambda _, val: dpg.set_value("math_channel_operators", val),
            )
            with dpg.filter_set(tag="math_channel_operators"):
                dpg.add_selectable(
                    parent="math_channel_operators",
                    label="+",
                    filter_key="+",
                    callback=save_operator,
                    user_data="+",
                )
                dpg.add_selectable(
                    parent="math_channel_operators",
        ...
        dpg.add_button(
            label="Create",
            callback=create_math_channel,
            user_data=custom_parameters_list,
        )
        custom_parameters_list = []
...
def create_math_channel(sender):
    ...
    if channel_name not in math_channels_dict:
        ...
        math_channels_dict[channel_name]["equation"] = copy.deepcopy(
            custom_parameters_list
        )
        math_channels_dict[channel_name]["placeholder_equation"] = copy.deepcopy(
            custom_parameters_list
        )
    ...
    dpg.add_selectable(
        parent="math_channels_list",
        label=channel_name,
        filter_key=channel_name,
        callback=add_plot_math,
        user_data=channel_name,
    )

    with open(
        math_channels_CSVFile_path, mode="a", newline="", encoding="utf-8"
    ) as csv_file:
        writer = csv.writer(csv_file)
...
def save_parameter_name(sender, app_data, pid):
    ...
    if dpg.does_item_exist("custom_parameter_equation") == False:
        dpg.add_text(
            parameters[pid]["name"],
            tag="custom_parameter_equation",
            parent="chosen_custom_parameters_group",
            color=COLORS["red"],
        )
    else:
        ...
        dpg.add_text(
            placeholder + " " + parameters[pid]["name"],
            tag="custom_parameter_equation",
            parent="chosen_custom_parameters_group",
            color=COLORS["red"],
        )
...
def evaluate_infix(expression):
    ...
    while i < len(expression):
        ...
        # make sure the token is a number
            ...
            if token == "-" and (
                i == 0 or expression[i - 1] in operators or expression[i - 1] == "("
            ):
                i += 1
                ...
                while (
                    ops
                    and ops[-1] != "("
                    and precedence_of(ops[-1]) >= precedence_of(token)
                ):
                    values.append(apply_operator(ops.pop(), values.pop(), values.pop()))
                ops.append(token)
        else:
...
def add_plot_math(sender, app_data, channel_name):
    ...
    if dpg.does_alias_exist(f"p_plot_{channel_name}"):
        dpg.remove_alias(f"p_plot_{channel_name}")
    if dpg.does_alias_exist(f"{channel_name}_x"):
        dpg.remove_alias(f"{channel_name}_x")
    if dpg.does_alias_exist(f"{channel_name}_y"):
        dpg.remove_alias(f"{channel_name}_y")
    if dpg.does_alias_exist(f"{channel_name}_series"):
        dpg.remove_alias(f"{channel_name}_series")
...
# ====================================================================================================
    ...
    if is_collumn_two:
        with dpg.collapsing_header(
            tag=f"{channel_name}_collapsing_header",
            label=channel_name,
            closable=True,
            default_open=True,
            ...
            with dpg.plot(
                ...
                dpg.add_plot_axis(
                    dpg.mvYAxis, label=channel_unit, tag=f"{channel_name}_y"
                )
                dpg.add_line_series(
                    list(math_channels_plot_data[channel_name]["x"]),
                    list(math_channels_plot_data[channel_name]["y"]),
                    label=channel_name,
                    parent=f"{channel_name}_y",
                    tag=f"{channel_name}_series",
                )
                dpg.add_plot_annotation(
                    label="0.0",
                    offset=(float("inf"), float("inf")),
                    tag=f"{channel_name}_value",
                )
        is_collumn_two = False
    ...
    with dpg.tab_bar(tag="tab-bar"):
        with dpg.tab(label="GopherCAN", tag="tab-gophercan"):
            with dpg.group(horizontal=True):
                ...
                dpg.add_checkbox(
                    tag="should_open_yaml", default_value=False, show=False
                )
                dpg.add_button(
                    label="Browse",
                    callback=lambda: dpg.set_value("should_open_yaml", True),
                )
            dpg.add_text(
                "No configuration loaded", tag="config_path", color=COLORS["red"]
            )
        ...
        with dpg.tab(label="Data Parser"):
            with dpg.group(horizontal=True):
                ...
                dpg.add_button(
                    ...
                    label="Convert",
                    enabled=False,
                    callback=lambda: dpg.set_value("convert_clicked", True),
                )
        ...
        with dpg.tab(label="Telemetry", tag="tab-telemetry"):
            with dpg.group(horizontal=True):
                ...
                dpg.add_button(
                    tag="clear_parameters",
                    label="Clear",
                    callback=clear_parameters,
                    enabled=False,
                )
                dpg.add_button(
                    tag="math_btn",
                    label="Math",
                    callback=math_btn_callback,
                    enabled=False,
                )
                dpg.add_button(
                    tag="math_channels_btn", label="Math Channels +", enabled=False
                )
                ...
                dpg.add_button(
                    tag="delete_preset", label="Delete Preset +", enabled=False
                )
                dpg.add_button(tag="settings_btn", label="Settings")

            with dpg.popup(
                "load_preset", no_move=True, mousebutton=dpg.mvMouseButton_Left
            ):
                dpg.add_input_text(
                    hint="Name",
                    callback=lambda _, val: dpg.set_value("offline_presets_list", val),
                )
                with dpg.filter_set(tag="offline_presets_list"):
                    pass

            with dpg.popup(
                "delete_preset", no_move=True, mousebutton=dpg.mvMouseButton_Left
            ):
                dpg.add_input_text(
                    hint="Name",
                    callback=lambda _, val: dpg.set_value("offline_presets_list", val),
                ...
                dpg.add_checkbox(
                    tag="load_preset_clicked", default_value=False, show=False
                )
                dpg.add_checkbox(
                    tag="save_preset_clicked", default_value=False, show=False
                )

            with dpg.popup("add_btn", no_move=True, mousebutton=dpg.mvMouseButton_Left):
                dpg.add_input_text(
                    hint="Name",
                    callback=lambda _, val: dpg.set_value("parameter_list", val),
                )
                with dpg.filter_set(tag="parameter_list"):
                    pass

            with dpg.popup(
                "math_channels_btn", no_move=True, mousebutton=dpg.mvMouseButton_Left
            ):
                dpg.add_input_text(
                    hint="Name",
                    callback=lambda _, val: dpg.set_value("math_channels_list", val),
                )
            ...
            with dpg.popup(
                ...
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_input_int(
                        tag="plot_length",
                        default_value=PLOT_LENGTH_S,
                        min_value=1,
                        min_clamped=True,
                        callback=set_plot_size,
                        width=100,
                    )
                with dpg.group(horizontal=True):
                ...
                with dpg.group(horizontal=True):
                    dpg.add_text("Port:")
                    dpg.add_combo(
                        items=["Serial Port", "Network Socket"],
                        callback=set_port_type,
                        width=150,
                    )
                ...
                if len(argv) < 2 or argv[1] != "host":
                    ...
                    with dpg.group(horizontal=True):
                        dpg.add_input_text(
                            tag="trackside_hostname",
                            hint="host",
                            default_value=T_HOSTNAME,
                            width=150,
                        )
                        dpg.add_button(label="Connect", callback=trackside_connect)
    dpg.add_progress_bar(
        tag="packet_loss_bar", default_value=0.0, width=100, pos=[700, 4]
    )
...
def change_theme():
    if dpg.does_alias_exist("plot_theme"):
        dpg.remove_alias("plot_theme")
...
# calculate the amount of packets lost
def packets_lost():
    global packets_lost_pct
    global yaml_open
    if not yaml_open:
        return 0.0
...
def update_plots():
    ...
    while True:
            ...
            for pname in math_channels_dict:
                for index, token in enumerate(math_channels_dict[pname]["equation"]):
                    if isinstance(token, list):
                        if id == token[0]:
                            math_channels_dict[pname]["placeholder_equation"][index] = (
                                str(value)
                            )
            # if plot is visible, update series
            if dpg.does_item_exist(f"{id}_series"):
                dpg.set_value(
                    f"{id}_series", [list(plot_data[id]["x"]), list(plot_data[id]["y"])]
                )
        ...
        # packets lost progress bar
        packet_loss_value = packets_lost()
        if dpg.does_item_exist("packet_loss_bar"):
            dpg.set_value("packet_loss_bar", packet_loss_value)
            # dpg.bind_item_theme(f"{id}_series", "plot_theme")
        for pname in math_channels_plot_data:
            ...
            if dpg.does_item_exist(f"{pname}_series"):
                dpg.set_value(
                    f"{pname}_series",
                    [
                        list(math_channels_plot_data[pname]["x"]),
                        list(math_channels_plot_data[pname]["y"]),
                    ],
                )
                dpg.set_item_label(
                    f"{pname}_value", round(math_channels_plot_data[pname]["y"][-1], 3)
                )
